...
class _EditSecondaryContactDetailsState
    extends State<EditSecondaryContactDetails> {
  StatesIn selectst;
            ...
            return AlertDialog(
                      ...
                      if (!snapshot.hasData)
                        ...
                        addressController
                            .value = new TextEditingController.fromValue(
                                new TextEditingValue(text: userDocument['add']))
                            .value;
                        cityController.value =
                            new TextEditingController.fromValue(
                                    new TextEditingValue(
                                        text: userDocument['city']))
                                .value;
                        postController.value =
                            new TextEditingController.fromValue(
                                    new TextEditingValue(
                                        text: userDocument['postal']))
                                .value;
                        contactController.value =
                            new TextEditingController.fromValue(
                                    new TextEditingValue(
                                        text: userDocument['contact']))
                                .value;
                        emailController.value =
                            new TextEditingController.fromValue(
                                    new TextEditingValue(
                                        text: userDocument['emailid']))
                                .value;
                        if (state == 0) {
                          selectst = _states[_states.indexWhere((_states) =>
                              _states.id.contains(userDocument['state']))];
              ...
              actions: <Widget>[
                ...
                new FlatButton(
                    ...
                    onPressed: () {
                      updateData(
                          snapshot.data.uid, "add", addressController.text);
                      updateData(snapshot.data.uid, "country", _country);
                      updateData(snapshot.data.uid, "state", selectst.id);
                      updateData(
                          snapshot.data.uid, "city", cityController.text);
                      updateData(
                          snapshot.data.uid, "postal", postController.text);
                      updateData(
                          snapshot.data.uid, "contact", contactController.text);
                      updateData(
                          snapshot.data.uid, "emailid", emailController.text);
  ...
  updateData(selectedDoc, field, newValues) {
    Firestore.instance
        ...
        .collection('myprofile')
        .document('secondarycontact')
        .updateData({field: newValues}).catchError((e) {
      print(e);
    });
  }
}
