...
import 'package:xmplaressflutter/menu/user_menu.dart';
...
class FirebaseFirestoreService {
  ...
  Future<User_info> createNote(String title, String description) async {
    final TransactionHandler createTransaction = (Transaction tx) async {
      final DocumentSnapshot ds = await tx.get(noteCollection.document());

      final User_info note = new User_info(ds.documentID, title, description);
    ...
    return Firestore.instance.runTransaction(createTransaction).then((mapData) {
      return User_info.fromMap(mapData);
    }).catchError((error) {
      print('error: $error');
      return null;
    });
  ...
  Future<dynamic> updateNote(User_info note) async {
